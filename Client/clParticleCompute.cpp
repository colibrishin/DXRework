...
namespace Client::ComputeShaders
{
	...
	void ParticleCompute::loadDerived()
	{
		...
		m_noises_[0] = std::make_shared<Resources::Texture2D>
				(f_name[0], Resources::Texture::GenericTextureDescription{});
		m_noises_[1] = std::make_shared<Resources::Texture2D>
				(f_name[1], Resources::Texture::GenericTextureDescription{});
		m_noises_[2] = std::make_shared<Resources::Texture2D>
				(f_name[2], Resources::Texture::GenericTextureDescription{});
	...
	)
	{
		...
		for (auto i = 0; i < count; ++i)
		{
			...
			world *= Matrix::CreateScale
					(config.GetParam<float>(Components::ParticleRenderer::size_slot)) * Matrix::CreateTranslation
					(new_pos - world.Translation());
