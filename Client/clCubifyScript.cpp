...
namespace Client::Scripts
{
	...
	void CubifyScript::UpdateCubes(bool normal)
	{
		...
		if (!LockWeak(GetOwner(), owner))
		{
			return;
		}
		if (!LockWeak(owner->GetScene(), scene))
		{
			return;
		}
		if (!LockWeak(scene->GetMainActor(), player))
		{
			return;
		}
	...
	eCubeType CubifyScript::GetCubeType() const
	{
		return m_cube_type_;
	}
	...
	{
		StrongScene scene;
		if (!LockWeak(GetSceneManager().GetActiveScene(), scene))
		{
			return;
		}
	...
	void CubifyScript::updateCubesImpl(bool normal)
	{
		...
		if (const auto& owner = GetOwner().lock())
		{
			...
			StrongCamera camera;
			if (!LockWeak(scene->GetMainCamera(), camera))
			{
				return;
			}
