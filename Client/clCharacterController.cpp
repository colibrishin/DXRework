...
BOOST_CLASS_EXPORT_IMPLEMENT(Engine::Abstract::StateController<Client::eCharacterState>)

SERIALIZER_ACCESS_IMPL(Client::State::CharacterController,
	_ARTAG(_BSTSUPER(Engine::Abstract::StateController<eCharacterState>))
	_ARTAG(m_offset_)
	_ARTAG(m_shoot_interval)
	_ARTAG(m_hp_)
)
...
namespace Client::State
{
	...
	void CharacterController::CheckJump(const boost::shared_ptr<Engine::Component::Rigidbody>& rb)
	{
		if (!rb->GetGrounded())
		{
	...
	void CharacterController::CheckMove(const boost::shared_ptr<Engine::Component::Rigidbody>& rb)
	{
		float speed = 1.0f;
