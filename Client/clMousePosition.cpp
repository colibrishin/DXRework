...
namespace Client::Object
{
  ...
  void MousePositionText::PreUpdate(const float& dt) { Text::PreUpdate(dt); }

  void MousePositionText::Update(const float& dt)
  {
    Text::Update(dt);
    const Vector2 pos = GetSceneManager()
                        .GetActiveScene()
                        .lock()
                        ->GetMainCamera()
                        .lock()
  ...
  void MousePositionText::PreRender(const float& dt) { Text::PreRender(dt); }

  void MousePositionText::Render(const float& dt) { Text::Render(dt); }

  void MousePositionText::PostRender(const float& dt) { Text::PostRender(dt); }
} // namespace Client::Object
