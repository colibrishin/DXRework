...
namespace Engine::Resources
{
	...
	void Material::OnImGui()
	{
		...
		if (ImGui::Button("Edit Resources"))
		{
			m_b_edit_dialog_ = true;
		}
								...
								if (ImGui::Selectable((*it)->GetName().c_str(), false))
								{
									std::erase_if
											(
											 ...
											 {
												 return pair.first == (*it)->GetName() && pair.second == (*it)->
												        GetMetadataPath();
											 }
											);
								...
								if (ImGui::Selectable(it->second->GetName().c_str(), false))
								{
									std::erase_if
											(
											 ...
											 [&it](const std::pair<EntityName, MetadataPathStr>& pair)
											 {
												 return pair.first == it->second->GetName() && pair.second == it->second
												        ->GetMetadataPath();
											 }
											);
	...
	void Material::SetTextureSlot(const std::string& name, const UINT slot)
	{
		...
		const auto it   = std::ranges::find_if
				(
				 texs, [&name](const StrongResource& res)
				 {
	...
	StructuredBuffer<SBs::MaterialSB>& Material::GetMaterialSBBuffer()
	{
