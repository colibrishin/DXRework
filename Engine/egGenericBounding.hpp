...
namespace Engine::Physics
{
  ...
  public:
    ...
    {
      type = other.type;
      if (type == BOUNDING_TYPE_BOX) { m_boundings_.box = other.m_boundings_.box; }
      else if (type == BOUNDING_TYPE_SPHERE) { m_boundings_.sphere = other.m_boundings_.sphere; }
    }
    ...
    void SetType(const eBoundingType type_) { type = type_; }

    void SetCenter(const Vector3& center_)
    {
      if (type == BOUNDING_TYPE_BOX) { m_boundings_.box.Center = center_; }
      else if (type == BOUNDING_TYPE_SPHERE) { m_boundings_.sphere.Center = center_; }
    }

    void SetExtents(const Vector3& extents_)
    {
      if (type == BOUNDING_TYPE_BOX) { m_boundings_.box.Extents = extents_; }
      else if (type == BOUNDING_TYPE_SPHERE) { m_boundings_.sphere.Radius = MaxElement(extents_); }
    }

    [[nodiscard]] Vector3 GetExtents() const
    {
      if (type == BOUNDING_TYPE_BOX) { return m_boundings_.box.Extents; }
      if (type == BOUNDING_TYPE_SPHERE) { return Vector3(m_boundings_.sphere.Radius); }
      throw std::exception("GetExtents: Invalid type");
    }

    [[nodiscard]] Quaternion GetOrientation() const
    {
      if (type == BOUNDING_TYPE_BOX) { return m_boundings_.box.Orientation; }
      if (type == BOUNDING_TYPE_SPHERE) { return Quaternion::Identity; }
    ...
    [[nodiscard]] bool __vectorcall Intersects(
      const GenericBounding& other, const Matrix& this_mat, const Matrix& other_mat, const float epsilon = g_epsilon
    ) const
    {
      if (type == BOUNDING_TYPE_BOX)
      {
    ...
    {
      if constexpr (std::is_same_v<BoundingType, BoundingOrientedBox>) { return other.Contains(m_boundings_.box); }
      else if constexpr (std::is_same_v<BoundingType, BoundingSphere>) { return other.Contains(m_boundings_.sphere); }
      else if constexpr (std::is_same_v<BoundingType, BoundingBox>) { return other.Contains(m_boundings_.box); }
      else
      {
  ...
  private:
    ...
    )
    {
      ...
      else { return lhs.Intersects(rhs.Center, dir, dist); }
    }
