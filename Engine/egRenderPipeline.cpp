...
namespace Engine::Manager::Graphics
{
  ...
  void RenderPipeline::DefaultRenderTarget(const Weak<CommandPair>& w_cmd) const
  {
  ...
  void RenderPipeline::InitializeRootSignature()
  {
    ...
    root_parameters[RASTERIZER_SLOT_SRV].InitAsDescriptorTable(1, &ranges[RASTERIZER_SLOT_SRV], D3D12_SHADER_VISIBILITY_ALL);
    root_parameters[RASTERIZER_SLOT_CB].InitAsDescriptorTable(1, &ranges[RASTERIZER_SLOT_CB], D3D12_SHADER_VISIBILITY_ALL);
    root_parameters[RASTERIZER_SLOT_UAV].InitAsDescriptorTable(1, &ranges[RASTERIZER_SLOT_UAV], D3D12_SHADER_VISIBILITY_ALL);
    root_parameters[RASTERIZER_SLOT_SAMPLER].InitAsDescriptorTable(1, &ranges[RASTERIZER_SLOT_SAMPLER], D3D12_SHADER_VISIBILITY_ALL);
  ...
  {
    const auto& cmd = GetD3Device().AcquireCommandPair(L"Cleanup").lock();
  ...
  void RenderPipeline::PostRender(const float& dt)
  {
    ...
    const auto& cmd = GetD3Device().AcquireCommandPair(L"Finalize Render").lock();
  ...
  void RenderPipeline::CopyBackBuffer(const Weak<CommandPair>& w_cmd, ID3D12Resource* resource) const
  {
    const auto& cmd = w_cmd.lock();
