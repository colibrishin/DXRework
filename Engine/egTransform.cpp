...
namespace Engine::Components
{
  ...
  void __vectorcall Transform::SetLocalPosition(const Vector3& position) { m_position_ = position; }

  void __vectorcall Transform::SetLocalRotation(const Quaternion& rotation) { m_rotation_ = rotation; }

  void __vectorcall Transform::SetLocalScale(const Vector3& scale) { m_scale_ = scale; }
  ...
  void Transform::SetSizeAbsolute(bool absolute) { m_b_s_absolute_ = absolute; }

  void Transform::SetRotateAbsolute(bool absolute) { m_b_r_absolute_ = absolute; }
  ...
  Quaternion Transform::GetWorldRotation() const
  {
    ...
    {
      if (!tr_c->m_b_r_absolute_) { world *= parent->GetLocalRotation(); }
  ...
  Vector3 Transform::GetLocalPosition() const { return m_position_ + GetAnimationPosition(); }

  Quaternion Transform::GetLocalRotation() const { return m_rotation_ * GetAnimationRotation(); }

  Vector3 Transform::GetLocalScale() const { return m_scale_ * GetAnimationScale(); }

  Vector3 Transform::GetAnimationPosition() const { return m_animation_position_; }

  Vector3 Transform::GetAnimationScale() const { return m_animation_scale_; }

  Quaternion Transform::GetAnimationRotation() const { return m_animation_rotation_; }
  ...
  Vector3 Transform::GetWorldScale() const
  {
    ...
    while (const auto parent = tr_p.lock())
    {
      if (!tr_c->m_b_s_absolute_) { world *= parent->GetLocalScale(); }
  ...
  Vector3 Transform::Forward() const { return Vector3::Transform(g_forward, GetWorldRotation()); }

  Vector3 Transform::Right() const { return Vector3::Transform(Vector3::Right, GetWorldRotation()); }

  Vector3 Transform::Up() const { return Vector3::Transform(Vector3::Up, GetWorldRotation()); }

  void Transform::Translate(Vector3 translation) { m_position_ += translation; }
  ...
  void Transform::OnDeserialized() { Component::OnDeserialized(); }
  ...
  Matrix Transform::GetWorldMatrix()
  {
    ...
    while (const auto parent = tr_p.lock())
    {
      ...
      if (tr_c->m_b_s_absolute_) { inv *= Matrix::CreateScale(parent->GetLocalScale()).Invert(); }
      if (tr_c->m_b_r_absolute_) { inv *= Matrix::CreateFromQuaternion(parent->GetLocalRotation()).Invert(); }
  ...
  void Transform::Unbind() { GetRenderPipeline().SetWorldMatrix({}); }
  ...
  WeakTransform Transform::FindNextTransform(const Transform& transform_)
  {
    ...
    {
      if (const auto transform = parent->GetComponent<Transform>().lock()) { return transform; }
