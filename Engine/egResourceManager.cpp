...
namespace Engine::Manager
{
	...
	void ResourceManager::OnImGui()
	{
		...
		OpenNewSimpleDialog<Resources::Texture2D>
				(m_b_imgui_load_texture_dialog_, Resources::Texture::GenericTextureDescription{});
		...
		if (m_b_imgui_load_shader_dialog_)
		{
			if (ImGui::Begin("New Shader", &m_b_imgui_load_shader_dialog_, ImGuiWindowFlags_AlwaysAutoResize))
			{
				...
				static const char* filter_func[] = {
					...
					"MinLinearMagMipPoint", "MinLinearMagPointMipLinear", "MinMagLinearMipPoint", "MinMagMipLinear",
					"Anisotropic"
				};
				...
				{
					"PointList", "LineList", "LineStrip", "TriangleList", "TriangleStrip", "LineListAdj",
					"LineStripAdj", "TriangleListAdj", "TriangleStripAdj", "PatchList"
				};
					...
					try
					{
						Resources::Shader::Create
								(
								 name_buffer,
								 path_buffer,
								 static_cast<eShaderDomain>(sel_domain),
								 static_cast<UINT>(sel_depth | sel_depth_func >> 2),
								 static_cast<UINT>(sel_cull | sel_fill >> 3),
								 static_cast<D3D12_FILTER>(sel_filter),
								 static_cast<UINT>(sel_sampler_address | sel_sampler_func >> 5),
								 &default_format, // todo: add a way to select this
								 1,
								 DXGI_FORMAT_D24_UNORM_S8_UINT,
								 static_cast<D3D_PRIMITIVE_TOPOLOGY>(sel_topology + 1),
								 static_cast<D3D12_PRIMITIVE_TOPOLOGY_TYPE>(sel_topology + 1)
								);
