...
namespace Engine::Manager
{
	void SceneManager::ChangeScene(const WeakScene& it)
	{
		...
		for (const auto& light : m_active_scene_.lock()->GetGameObjects((eLayerType)LAYER_LIGHT))
		{
			Graphics::ShadowManager::GetInstance().RegisterLight(light.lock()->GetSharedPtr<Objects::Light>());
		}
	}
