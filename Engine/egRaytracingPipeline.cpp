...
namespace Engine::Manager::Graphics
{
	...
	void RaytracingPipeline::Render(const float& dt)
	{
		...
		for (const auto& tex : m_used_textures_)
		{
			tex->ManualTransition
					(cmd->GetList(), D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_ALL_SHADER_RESOURCE);
		}
	...
	void RaytracingPipeline::InitializeShaderTable()
	{
...

		{
			...
			m_closest_hit_shader_table_size_ = Align
					(sizeof(HitShaderRecord), D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT);
		}
	...
	void RaytracingPipeline::UpdateHitShaderRecords()
	{
		...
		const auto& new_size = Align
				(m_hit_shader_records_.size() * sizeof(HitShaderRecord), D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT);
	...
	void RaytracingPipeline::BuildTLAS(
		...
		std::vector<StructuredBuffer<SBs::InstanceSB>>&             instance_sb
	)
	{
		const auto& default_heap = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
		...
		for (const auto& [w_mtr, instances] : instances)
		{
			if (const StrongMaterial& mtr = w_mtr.lock())
			{
				if (const StrongModel& shape = mtr->GetResource<Resources::Shape>(0).lock())
				{
					...
					StructuredBuffer<SBs::MaterialSB>& material_sb = mtr->GetMaterialSBBuffer();
		...
		m_tlas_.instanceDescPool.Update
				(instance_descs.data(), sizeof(D3D12_RAYTRACING_INSTANCE_DESC), instance_descs.size());

		constexpr D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS build_flags =
		...
		const auto& result_size = Align
				(tlas_prebuild_info.ResultDataMaxSizeInBytes, D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BYTE_ALIGNMENT);
		const auto& scratch_size = Align
				(tlas_prebuild_info.ScratchDataSizeInBytes, D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BYTE_ALIGNMENT);
