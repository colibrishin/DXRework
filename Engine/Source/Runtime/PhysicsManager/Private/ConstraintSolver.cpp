...
namespace Engine::Managers
{
...
#ifdef PHYSX_ENABLED
#else
		auto& infos = CollisionDetector::GetInstance().GetCollisionInfo();
	...
	void ConstraintSolver::ResolveCollision(const Weak<Abstracts::ObjectBase>& p_lhs, const Weak<Abstracts::ObjectBase>& p_rhs)
	{
		auto lhs = p_lhs;
		auto rhs = p_rhs;
		...
		if (rb && rb_other)
		{
			...
			if (!GJKExtension::GetPenetration(cl, cl_other, lhs_normal, lhs_pen))
			{
				return;
			}
	...
	void ConstraintSolver::ResolveSpeculation(const Weak<Abstracts::ObjectBase>& p_lhs, const Weak<Abstracts::ObjectBase>& p_rhs)
	{
		...
		while (iter < CFG_SPECULATION_BISECTION_MAX_ITERATION)
		{
			if (lbnd.TestRay(rbnd, dir, distance))
			{
				distance *= f;
