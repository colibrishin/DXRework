...
namespace Engine
{
    inline void __vectorcall EvalImpulse(
    ...
    )
    {
        ...
        const Vector3 angVel2 = rv2.Cross(rel2);
        ...
        const Vector3 inertia1 =
                (MathExtension::XMTensorCross(inertiaT1, rel1.Cross(normal)).Cross(rel1));
        const Vector3 inertia2 =
                (MathExtension::XMTensorCross(inertiaT2, rel2.Cross(normal)).Cross(rel2));

        const float angular_impulse = (inertia1 + inertia2).Dot(normal);

        const float j = (-(1.0f + CFG_RESTITUTION_COEFFICIENT) * impulse_force) / (total_mass + angular_impulse);
        ...
        angular1 = MathExtension::XMTensorCross(inertiaT1, rel1.Cross(-impulse));
        angular2 = MathExtension::XMTensorCross(inertiaT2, rel2.Cross(impulse));
    }
}
