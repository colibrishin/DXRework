...
namespace Engine::Resources
{
...
#if defined(USE_DX12)
	ID3D12DescriptorHeap* Texture::GetSRVDescriptor() const
	{
		return m_srv_.Get();
	}

	ID3D12DescriptorHeap* Texture::GetRTVDescriptor() const
	{
		return m_rtv_.Get();
	}

	ID3D12DescriptorHeap* Texture::GetDSVDescriptor() const
	{
		return m_dsv_.Get();
	}

	ID3D12DescriptorHeap* Texture::GetUAVDescriptor() const
	{
		return m_uav_.Get();
	}
	...
	void Texture::Bind(
		const Weak<CommandPair>& w_cmd, const DescriptorPtr& w_heap, const eBindType type, const UINT slot,
		const UINT               offset
	) const
	{
	...
	void Texture::mapInternal()
	{
		...
		Managers::D3Device::GetInstance().GetDevice()->GetCopyableFootprints(&native_desc, 0, 1, 0, &footprint, &rows, nullptr, &total_bytes);
	...
	const Texture::GenericTextureDescription& Texture::GetDescription() const
	{
		return m_desc_;
	}
	...
	void Texture::Load_INTERNAL()
	{
		if ((m_desc_.Flags & D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL) &&
		    (m_desc_.Flags & D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS))
		{
		...
		if (!GetPath().empty())
		{
			Managers::D3Device::GetInstance().CreateTextureFromFile
					(
					 absolute(GetPath()),
		...
		else
		{
			...
			const D3D12_CLEAR_VALUE* cv_ptr = (
			                                  ...
			                                  (m_desc_.Flags & D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL) ?
				                                  &clear_value :
				                                  nullptr;
		...
		if (m_custom_desc_[1] && m_desc_.Flags & D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL)
		{
			Managers::D3Device::GetInstance().GetDevice()->CreateDepthStencilView
					(m_res_.Get(), &m_dsv_desc_, m_dsv_->GetCPUDescriptorHandleForHeapStart());
		}
		else if (m_desc_.Flags & D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL)
		{
			Managers::D3Device::GetInstance().GetDevice()->CreateDepthStencilView
					(m_res_.Get(), nullptr, m_dsv_->GetCPUDescriptorHandleForHeapStart());
		}
		...
		if (m_custom_desc_[2] && m_desc_.Flags & D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS)
		{
			Managers::D3Device::GetInstance().GetDevice()->CreateUnorderedAccessView
					(m_res_.Get(), nullptr, &m_uav_desc_, m_uav_->GetCPUDescriptorHandleForHeapStart());
		}
		else if (m_desc_.Flags & D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS)
		{
			Managers::D3Device::GetInstance().GetDevice()->CreateUnorderedAccessView
					(m_res_.Get(), nullptr, nullptr, m_uav_->GetCPUDescriptorHandleForHeapStart());
		}
