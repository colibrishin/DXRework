...
namespace Engine
{
	...
	enum ENGINE_CORE_API eMouseButtonEnum
	{
		MOUSE_LEFT,
		MOUSE_MIDDLE,
		MOUSE_RIGHT,
	...
	struct ENGINE_CORE_API MouseState
	{
		bool    leftButton;
		bool    middleButton;
		bool    rightButton;
...
#pragma pack (push, 1)
	namespace Keyboard 
	{
		enum ENGINE_CORE_API Keys : unsigned char
		{
			None = 0,
	...
	struct ENGINE_CORE_API KeyboardState
	{
		bool Reserved0 : 8;
...
#pragma clang diagnostic ignored "-Wunsafe-buffer-usage"
		...
		bool IsKeyDown(Keyboard::Keys key) const noexcept
		{
			if (key <= 0xfe)
			{
		...
		bool IsKeyUp(Keyboard::Keys key) const noexcept
		{
			if (key <= 0xfe)
			{
				auto ptr = reinterpret_cast<const uint32_t*>(this);
...
#pragma pack(pop)
	...
	struct ENGINE_CORE_API IInputAPI
	{
		...
		virtual bool HasScrollWheelChanged() const noexcept = 0;
		virtual int  GetScrollWheelValue() const noexcept = 0;
		virtual int  GetPreviousScrollWheelValue() const noexcept = 0;

		virtual int GetMouseX() const noexcept = 0;
		virtual int GetMouseY() const noexcept = 0;

		virtual bool IsKeyDown(const Keyboard::Keys key) const noexcept = 0;
		virtual bool IsKeyPressed(const Keyboard::Keys key) const noexcept = 0;
		virtual bool IsKeyReleased(const Keyboard::Keys key) const noexcept = 0;
		virtual bool IsKeyDown(const eMouseButtonEnum key) const noexcept = 0;
		virtual bool IsKeyPressed(const eMouseButtonEnum key) const noexcept = 0;
