...
namespace Engine
{
    ...
    public:
        using PoolVector = std::vector<WeakT, u_pool_allocator_single<WeakT>>;

        octree_impl()
            : m_pointer_(nullptr),
              m_parent_(nullptr),
              m_bounds_(Vector3::Zero,
                        map_size_vec),
              // Root node is the whole map
              ...
              m_life_count_(other.m_life_count_)
        {
            ...
            for ( int i = 0; i < octant_count; ++i )
            {
                ...
                {
                    m_children_[ i ]            = allocate(*other.m_children_[ i ]);
                    m_children_[ i ]->m_parent_ = this;
                }
            }
        }

        [[nodiscard]] const PoolVector& Read() const
        {
            return m_values_;
        }
        ...
        {
            return m_bounds_.Intersects(point,
                                        dir,
                                        distance);
        }
        ...
        {
            return std::sqrtf(Vector3::DistanceSquared(m_bounds_.Center,
                                                       point));
        }
        ...
        {
            // attempt to insert to the outside of the map
            if ( this == nullptr )
            {
                return false;
            }
            ...
            while ( !stack.empty() )
            {
                ...
                const std::bitset<3> floor_bit((extent_scale.x <= smallest_scale) + (
                ...
                if ( bounding_value.ContainsBy(node_bound) == DirectX::CONTAINS )
                {
                    ...
                    for ( int i = 0; i < octant_count; ++i )
                    {
                        ...
                        if ( node_children[ i ] )
                        {
                            if ( bounding_value.ContainsBy(node_children[ i ]->m_bounds_) == DirectX::CONTAINS )
                            {
                        ...
                        {
                            const auto bound = GetBound(node_extent,
                                                        node_center,
                                                        static_cast<eOctant>( i ));
                            ...
                            if ( bound_check == DirectX::ContainmentType::CONTAINS )
                            {
                                ...
                                node_children[ i ]            = allocate(bound);
            ...
            // there is no match with current object (oob)
            return false;
        }
                    ...
                    if ( !node_value.empty() )
                    {
                        ...
                        for ( auto it = node_value.begin(); it != node_value.end(); )
                        {
                            ...
                            if ( bound_check != DirectX::ContainmentType::CONTAINS )
                            {
                                ...
                                if ( !cursor )
                                {
                                    // panic: Even root cannot contain the object, need to rebuild the whole tree.
                                    root()->Panic();
                                    return;
                                }
        ...
        [[nodiscard]] std::vector<WeakT> Hitscan(const Vector3& point,
            ...
            while ( !q.empty() )
            {
                ...
                if ( expanded )
                {
                    for ( const auto& v : value )
                    {
                        ...
                        {
                            if ( const auto& bounding = BoundingValueGetter::value(locked);
                                bounding.TestRay(point,
                                                 direction,
                                                 dist) )
                            {
                                if ( !FloatCompare(distance,
                                                   0.f) )
                                {
                                    if ( dist > distance )
                                    {
    ...
    private:
        ...
        {
            return std::fabs(a - b) < Epsilon * std::fmaxf(1.0f,
                                                           std::fmaxf(std::fabsf(a),
                                                                      std::fabsf(b)));
        }

        explicit octree_impl(const BoundingBox& bounds)
            : m_pointer_(nullptr), m_parent_(nullptr), m_b_initialized_(false), m_life_count_(node_lifespan)
        {
            m_bounds_ = bounds;
        }

        octree_impl(const BoundingBox& bounds, const PoolVector& values)
            : m_pointer_(nullptr), m_parent_(nullptr), m_b_initialized_(false), m_life_count_(node_lifespan)
        {
            m_bounds_ = bounds;
            m_values_ = values;
        ...
        void Build()
        {
            ...
            while ( !stack.empty() )
            {
                ...
                const std::vector<BoundingBox> octants = GetBounds(node_extent,
                                                                   node_center);
                std::array<PoolVector, octant_count> octant_values;
                ...
                for ( int i = 0; i < octant_count; ++i )
                {
                    ...
                    {
                        node_children[ i ]            = allocate(octants[ i ], octant_values[ i ]);
                        node_children[ i ]->m_parent_ = node;
                        node_children[ i ]->Build();
                        node_active_children.set(i);
                        stack.push(node_children[ i ].get());
        ...
        static std::vector<BoundingBox> __vectorcall GetBounds(const Vector3& extent, const Vector3& center)
        {
            ...
            BoundingBox::CreateFromPoints(tlfb,
                                          center + (extent * tlf),
                                          center);
            BoundingBox::CreateFromPoints(trfb,
                                          center + (extent * trf),
                                          center);
            BoundingBox::CreateFromPoints(blfb,
                                          center + (extent * blf),
                                          center);
            BoundingBox::CreateFromPoints(brfb,
                                          center + (extent * brf),
                                          center);

            BoundingBox::CreateFromPoints(tlbb,
                                          center,
                                          center + (extent * tlb));
            BoundingBox::CreateFromPoints(trbb,
                                          center,
                                          center + (extent * trb));
            BoundingBox::CreateFromPoints(blbb,
                                          center,
                                          center + (extent * blb));
            BoundingBox::CreateFromPoints(brbb,
                                          center,
                                          center + (extent * brb));
        ...
        static BoundingBox __vectorcall GetBound(const Vector3& extent, const Vector3& center, eOctant region)
        {
            ...
            switch ( region )
            {
                case eOctant::TopLeftFront:
                    BoundingBox::CreateFromPoints(bound,
                                                  center + (extent * tlf),
                                                  center);
                    break;
        ...
        struct octree_deleter;
        using unique_octree = Unique<octree_impl, octree_deleter>;
        ...
        std::array<unique_octree, octant_count>             m_children_;
        std::bitset<octant_count>                           m_active_children_;
        std::vector<WeakT, u_pool_allocator_single<WeakT> > m_values_;

        BoundingBox m_bounds_;

        std::queue<WeakT, std::deque<WeakT, u_pool_allocator_single<WeakT> > > m_insertion_queue_;
    ...
    void octree_impl<WeakT, BoundingValueGetter, Epsilon>::octree_deleter::operator()(octree_impl* ptr)
    {
        if ( ptr->m_pointer_ )
        {
