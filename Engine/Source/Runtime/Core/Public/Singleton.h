...
{
	ECLASS()
	class ENGINE_CORE_API SingletonBase : public Renderable
	{
		GENERATE_BODY
	public:
		SingletonBase();
...
namespace Engine::Abstracts
{
	...
	protected:
		Singleton() : SingletonBase()
		{
			...
			static_assert(SingletonChecker::ctor_triv, "Singleton should not trivially constructable");
			static_assert(SingletonChecker::ctor_void, "Singleton should not have default constructor");
			static_assert(SingletonChecker::ctor_copy, "Singleton should not have copy constructor");
		}
	...
	private:
		struct SingletonChecker final
		{
			...
			constexpr static bool ctor_triv = !std::is_trivially_constructible_v<T>;
			constexpr static bool ctor_copy = !std::is_copy_constructible_v<T>;
