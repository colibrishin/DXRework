...
namespace Engine::Components
{
	...
	void ParticleRenderer::Initialize()
	{
		...
		const auto& cmd = Managers::D3Device::GetInstance().AcquireCommandPair(L"Particle Renderer Init").lock();
		cmd->SoftReset();
	...
	void ParticleRenderer::Update(const float& dt)
	{
		...
		{
			const auto& cmd  = Managers::D3Device::GetInstance().AcquireCommandPair(L"Particle Renderer").lock();
