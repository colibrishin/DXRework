...
#define RESOURCE_SELF_INFER_GETTER_DECL(TYPE)                                         \
  ...
  static boost::weak_ptr<TYPE> GetByMetadataPath(const boost::filesystem::path& path);  \
  static boost::weak_ptr<TYPE> GetByRawPath(const boost::filesystem::path& path);
...
#define RESOURCE_SELF_INFER_GETTER_IMPL(TYPE)										  \
	...
	boost::weak_ptr<TYPE> TYPE::GetByMetadataPath(const boost::filesystem::path& path) { return Engine::Managers::ResourceManager::GetInstance().GetResourceByMetadataPath<TYPE>(path); } \
	boost::weak_ptr<TYPE> TYPE::GetByRawPath(const boost::filesystem::path& path) { return Engine::Managers::ResourceManager::GetInstance().GetResourceByRawPath<TYPE>(path); }


// Creatable resource creator which infers self as type
#define RESOURCE_SELF_INFER_CREATE_DECL(TYPE)                                     \
    static boost::shared_ptr<TYPE> Create(const std::string& name, const boost::filesystem::path& path);
...
namespace Engine::Abstracts
{
	...
	protected:
		Resource(boost::filesystem::path path, eResourceType type);
