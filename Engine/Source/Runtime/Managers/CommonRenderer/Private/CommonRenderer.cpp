#include "../Public/CommonRenderer.h"
...
{
	void __fastcall BuildRenderMap(
	...
	)
	{
		...
		{
			const auto& rcs = scene->GetCachedComponents<Components::RenderComponent>();

			tbb::parallel_for_each
					(
					 rcs.begin(), rcs.end(), [&out_map, &instance_count](const Weak<Abstracts::Component>& ptr_rc)
					 {
						 ...
						 const auto rc = ptr_rc.lock()->GetSharedPtr<Components::RenderComponent>();
						 ...
						 case RENDER_COM_T_MODEL:
							 PremapModelImpl(rc->GetSharedPtr<Components::ModelRenderer>(), out_map, instance_count);
							 break;
						 case RENDER_COM_T_PARTICLE:
							 PremapParticleImpl
									 (rc->GetSharedPtr<Components::ParticleRenderer>(), out_map, instance_count);
							 break;
