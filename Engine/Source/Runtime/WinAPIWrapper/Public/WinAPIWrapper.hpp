...
namespace WinAPI
{
    ...
    public:
        ...
        static void Update();
        static HWND GetHWND();
        static void RegisterHandler( const std::string_view                                      name,
                                     const std::function<LRESULT( HWND, UINT, WPARAM, LPARAM )>& func );
    ...
    private:
        ...
        static WinAPIWrapper& GetInstance()
        {
            return *s_instance_;
        }
        ...
        std::vector<std::pair<std::string, std::function<LRESULT( HWND, UINT, WPARAM, LPARAM )>>>
                m_registered_handlers_;
