...
)
{
	...
	const GraphicInterfaceContextReturnType& context     = g_graphic_interface.GetInterface().GetNewContext(D3D12_COMMAND_LIST_TYPE_DIRECT, false, L"Texture mapping");
	const GraphicInterfaceContextPrimitive&  primitive   = context.GetPointers();
...
{
	const GraphicInterfaceContextReturnType& context = g_graphic_interface.GetInterface().GetNewContext(D3D12_COMMAND_LIST_TYPE_COPY, false, L"Texture Copy");
	const GraphicInterfaceContextPrimitive& primitive = context.GetPointers();
