...
namespace Engine
{
    ...
    void D3D12GraphicPrimitiveShader::Generate( const Abstracts::Resource* resource, void* pipeline_signature )
	{
		...
		dev->CreateSampler(&sd, m_sampler_descriptor_heap_->GetCPUDescriptorHandleForHeapStart());
		...
		SetNativeSampler(m_sampler_descriptor_heap_.Get());
	}

	std::vector<std::pair<D3D12_INPUT_ELEMENT_DESC, std::string>> D3D12GraphicPrimitiveShader::GenerateInputDescription(
		ID3DBlob* blob
	)
	{
	...
	ID3D12PipelineState* D3D12GraphicPrimitiveShader::GetPSO() const
	{
		return m_pipeline_state_.Get();
	}

	ID3D12DescriptorHeap* D3D12GraphicPrimitiveShader::GetSamplerHeap() const
	{
