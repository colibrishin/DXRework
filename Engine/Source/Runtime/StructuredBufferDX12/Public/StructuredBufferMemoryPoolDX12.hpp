	...
	private:
		...
		void UpdateSizeIfNeeded(const size_t count)
		{
			if (m_allocated_size_ < count)
			{
				...
				const auto& cmd = Managers::D3Device::GetInstance().AcquireCommandPair(L"Allocation").lock();
		...
		void Copy(const SBType* src_data, const size_t offset, const size_t count)
		{
			...
			const auto& cmd = Managers::D3Device::GetInstance().AcquireCommandPair(L"Update").lock();
