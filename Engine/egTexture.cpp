...
namespace Engine::Resources
{
  Texture::Texture(std::filesystem::path path)
    : Resource(std::move(path), RES_T_TEX),
      m_texture_desc_() { Texture::Initialize(); }

  UINT Texture::GetWidth() const { return m_texture_desc_.Width; }

  UINT Texture::GetHeight() const { return m_texture_desc_.Height; }

  void Texture::SetSlot(eTexBindSlot slot, UINT slot_offset) { m_bound_slot_ = slot + slot_offset; }
  ...
  void Texture::PostRender(const float& dt) { GetRenderPipeline().UnbindResource(m_bound_slot_, SHADER_PIXEL); }
