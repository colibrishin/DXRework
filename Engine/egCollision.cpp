...
namespace Engine::Physics
{
	...
	namespace Ray
	{
		bool TestAxis(const Vector3& axis, const Vector3& delta, const Vector3& dir, const float& min, const float& max, float& tMin, float& tMax)
		{
		...
		bool TestRayOBBIntersection(const Vector3& origin, const Vector3& dir, const Vector3& aabb_min, const Vector3& aabb_max, const Matrix& world, float& intersection_distance)
		{
			float tMin = 0.f;
			float tMax = FLT_MAX;
		...
		bool SolveQuadratic(const float& a, const float& b, const float& c, float& x0, float& x1)
		{
			...
			{
				const float q = (b > 0) ? -0.5f * (b + sqrt(discr)) : -0.5f * (b - sqrt(discr));
				x0 = q / a;
				x1 = c / q;
			}
		...
		bool TestRaySphereIntersection(const DirectX::SimpleMath::Ray& ray, const Vector3& center, float radius, float& intersection_distance)
		{
