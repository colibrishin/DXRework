...
namespace DX
{
  ...
  void ThrowIfFailed(HRESULT hr) { if (FAILED(hr)) { throw com_exception(hr); } }
}
...
namespace FMOD::DX
{
  ...
  void ThrowIfFailed(FMOD_RESULT hr) { if (hr != FMOD_OK) { throw fmod_exception(hr); } }
}
