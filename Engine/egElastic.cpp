...
namespace Engine::Physics
{
        ...
        Vector3&          penetrationB)
    {
        ...
        const Vector3 inertia1 =
                (XMTensorCross(inertiaT1, rel1.Cross(normal)).Cross(rel1));
        const Vector3 inertia2 =
                (XMTensorCross(inertiaT2, rel2.Cross(normal)).Cross(rel2));

        const float angular_impulse = (inertia1 + inertia2).Dot(normal);

        const float j = (-(1.0f + g_restitution_coefficient) * impulse_force) /
                        (total_mass + angular_impulse);
