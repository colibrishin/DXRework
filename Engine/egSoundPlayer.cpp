...
{
    SoundPlayer::SoundPlayer(const WeakObject& owner) : Component(COMP_T_SOUND_PLAYER, owner) {}
    ...
    void SoundPlayer::Update(const float& dt)
    {
        ...
        if (!m_sound_->IsPlaying(GetOwner()))
        {
            return;
        }
    ...
    SoundPlayer::SoundPlayer() : Component(COMP_T_SOUND_PLAYER, {}) {}
}
