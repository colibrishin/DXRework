...
namespace Engine::Physics
{
  Vector3 __vectorcall EvalFriction(const Vector3& vel, float mu, float dt)
  {
    ...
    if (vel.Length() < mu * dt) { return invVel * dt; }
  ...
  )
  {
    ...
    if (!IsSamePolarity(evaluated_velocity.x, Undo.x)) { evaluated_velocity.x = 0.0f; }
    if (!IsSamePolarity(evaluated_velocity.y, Undo.y)) { evaluated_velocity.y = 0.0f; }
    if (!IsSamePolarity(evaluated_velocity.z, Undo.z)) { evaluated_velocity.z = 0.0f; }
  }
} // namespace Engine::Physics
